...
import { buildSwaggerDarkModeCssV2, buildSwaggerDarkModeScript } from './utils/swagger-dark-mode';
...
export class SwaggerService {
    ...
    private pinAuthTagsToTop(document: unknown): void {
        ...
        const existingByName = new Map<string, { name: string; description?: string }>();
    ...
    private buildFilteredDocument(document: unknown, includeMgmt: boolean): SwaggerDoc {
        ...
        if ((source.tags ?? []).length > 0) {
            source.tags = (source.tags ?? []).filter(tag => tagsInPaths.has(tag.name));
    ...
    private buildSwaggerUiOptions(
        adapter: {
            get?: (
                ...
                h: (
                    req: unknown,
                    res: { setHeader: (a: string, b: string) => void; send: (b: string) => void },
                ) => void,
            ) => void;
        },
        ...
        const customJsList = [`/${docsPath}/multitoken.js`, `/${docsPath}/swagger-dark-mode.js`];
        if (tagNamesSameAsGroup.length > 0) {
            const tagPathHighlightScript = buildTagPathHighlightScript(tagNamesSameAsGroup);
            if (adapter.get) {
    ...
    setupSwagger(app: INestApplication): void {
        ...
        const multiTokenScript = createMultiTokenScript(this.env.appName, multiTokenEnums);
        ...
        SwaggerModule.setup(
            ...
            this.buildSwaggerUiOptions(adapter, 'api-docs', publicDocument, multiTokenScript),
        );
        ...
        SwaggerModule.setup(
            ...
            this.buildSwaggerUiOptions(adapter, 'api-docs-mgmt', mgmtDocument, multiTokenScript),
        );
    }
